...
| file | IMPORTS | BODY |
|---|---|---|
| `solid.tsx` | *(none)* | `<Button>Save changes</Button>` |
| `outline.tsx` | *(none)* | `<Button variant="outline">Cancel</Button>` |
| `secondary.tsx` | *(none)* | `<Button variant="secondary">Export</Button>` |
| `ghost.tsx` | *(none)* | `<Button variant="ghost">Learn more</Button>` |
| `destructive.tsx` | `import { Trash2 } from "lucide-react";` | `<Button variant="destructive"><Trash2 />Delete</Button>` |
| `link.tsx` | *(none)* | `<Button variant="link">View docs</Button>` |
| `size-xs.tsx` | *(none)* | `<Button size="xs">Extra small</Button>` |
| `size-sm.tsx` | *(none)* | `<Button size="sm">Small</Button>` |
| `size-lg.tsx` | *(none)* | `<Button size="lg">Large</Button>` |
...
(When IMPORTS is *(none)*, delete the `IMPORTS` line entirely.)
...
```js
  execFileSync(
    "node",
    ["--experimental-strip-types", "--disable-warning=MODULE_TYPELESS_PACKAGE_JSON", "scripts/gen-mcp-data.mjs"],
