...
export const data: McpData = JSON.parse(
  readFileSync(join(here, "data.generated.json"), "utf8"),
)
```
...
if (!existsSync(genPath)) {
  ...
  execFileSync("node", ["scripts/gen-mcp-data.mjs"], {
    cwd: root,
    stdio: "ignore",
  })
  ...
  for (const c of data.components)
    if (!c.source || c.source.length < 10)
      errors.push(`component ${c.name} has empty source`)
}
...
const data: McpData = {
  components: [
    ...
    },
    {
      ...
      deps: [],
      install: "npx shadcn@latest add @byronwade/table",
      source: "x",
    },
  ],
  rule: "",
...
describe("searchComponents", () => {
  ...
  it("includes the install command and deps", () => {
    expect(searchComponents(data, { query: "button" })).toContain(
...
describe("getComponentSource", () => {
  it("returns the source for a known component", () => {
    expect(getComponentSource(data, { name: "button" })).toContain(
      "export const Button",
    )
  })
...
describe("checkOnSystem", () => {
  ...
  it("respects offSystemComponents off", () => {
    const r = checkOnSystem({
      code: `const x = <button>go</button>;`,
      offSystemComponents: "off",
    })
...
export function registerTools(server: Registrar, data: McpData): void {
  server.registerTool(
    ...
    {
      description:
        "Find byronwade/ui components by name or use-case. Returns description, install command, and deps.",
      inputSchema: {
        query: z.string().describe("name or use-case keywords; empty for all"),
      },
    },
  ...
  server.registerTool(
    "get_component_source",
    {
      description: "Return a byronwade/ui component's .tsx source.",
      inputSchema: { name: z.string().describe("component slug, e.g. button") },
    },
  ...
  server.registerTool(
    ...
    {
      description:
        "Check whether a TSX snippet is on-system (tokens/primitives/house utilities). Returns violations with line numbers.",
      inputSchema: {
        code: z.string(),
        offSystemComponents: z.enum(["warn", "error", "off"]).optional(),
      },
    },
    async (a) =>
      text(
        checkOnSystem({
          code: String(a.code ?? ""),
          offSystemComponents: a.offSystemComponents as
            | "warn"
            | "error"
            | "off"
            | undefined,
  ...
  server.registerTool(
    ...
    {
      description:
        "Return the byronwade/ui design rule (the constraints to follow when generating UI).",
      inputSchema: {},
    },
  ...
  server.registerTool(
    ...
    {
      description:
        "List byronwade/ui design tokens (names + OKLCH values). Never hardcode color.",
      inputSchema: {},
    },
  ...
  server.registerTool(
    ...
    {
      description:
        "List byronwade/ui house utilities (bg-grid, glow-brand, …). Reuse instead of re-rolling gradients/grids.",
      inputSchema: {},
    },
