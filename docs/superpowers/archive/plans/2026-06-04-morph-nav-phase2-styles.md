...
describe("MorphSidebar", () => {
  ...
  it("fires onSelect when an item is chosen", async () => {
    ...
    render(
      <MorphSidebar
        items={[{ id: "home", label: "Home", icon: Home, onSelect }]}
      />,
    )
  ...
  it("has no axe violations", async () => {
    const { container } = render(
      <MorphSidebar items={items} brand={<span>UI</span>} />,
    )
...
```json
{
  ...
  "files": [
    {
      "path": "registry/ui/morph-sidebar.tsx",
      "type": "registry:ui",
      "target": "components/ui/morph-sidebar.tsx"
    }
  ]
}
```
...
describe("MorphTabs", () => {
  it("renders every tab, marking the active one", () => {
    ...
    for (const i of items)
      expect(screen.getByRole("button", { name: i.label })).toBeInTheDocument()
  ...
  it("has no axe violations", async () => {
    const { container } = render(
      <MorphTabs items={items} sheet={<div>sheet</div>} />,
    )
    expect(await axe(container)).toHaveNoViolations()
  })
})
...
```json
{
  ...
  "files": [
    {
      "path": "registry/ui/morph-tabs.tsx",
      "type": "registry:ui",
      "target": "components/ui/morph-tabs.tsx"
    }
  ]
}
```
...
describe("MorphMenubar", () => {
  it("renders every top-level menu", () => {
    ...
    for (const m of menus)
      expect(screen.getByRole("button", { name: m.label })).toBeInTheDocument()
  })
        ...
        menus={[
          {
            id: "file",
            label: "File",
            items: [{ id: "new", label: "New", onSelect }],
          },
        ]}
      />,
    )
...
export function MorphMenubar({
  ...
  const activeMenu = active
    ? (menus.find((m) => m.id === active.id) ?? null)
    : null
...
```json
{
  ...
  "files": [
    {
      "path": "registry/ui/morph-menubar.tsx",
      "type": "registry:ui",
      "target": "components/ui/morph-menubar.tsx"
    }
  ]
}
```
...
const items = [
  { id: "files", label: "Files", icon: Files, panel: <div>files panel</div> },
  {
    ...
    label: "Search",
    icon: Search,
    panel: <div>search panel</div>,
  },
  {
    id: "git",
    label: "Source Control",
    icon: GitBranch,
    panel: <div>git panel</div>,
  },
]
      ...
      panel={
        <div className="flex h-full" style={{ width: expandedWidth }}>
          <div className="min-w-0 flex-1 overflow-auto border-r border-border p-4">
            {activeItem ? (
              <>
                <p className="mb-2 text-sm font-medium tracking-tight">
                  {activeItem.label}
                </p>
                {activeItem.panel}
              </>
...
```json
{
  ...
  "files": [
    {
      "path": "registry/ui/morph-rail.tsx",
      "type": "registry:ui",
      "target": "components/ui/morph-rail.tsx"
    }
  ]
}
```
