...
describe("getVariants", () => {
  it("synthesizes a single default variant when none are authored", () => {
    const doc = {
      slug: "x", name: "X", category: "UI", description: "", examples: [],
    } as ComponentDoc;
  ...
  it("carries the doc's type-level tags onto the synthetic default", () => {
    const doc = {
      slug: "y", name: "Y", category: "UI", description: "", examples: [],
      tags: ["a", "b"],
    } as ComponentDoc;
  ...
  it("every catalog component yields ≥1 variant with a kebab-case id", () => {
    for (const c of components) {
      ...
      for (const variant of v) expect(variant.id).toMatch(/^[a-z0-9]+(-[a-z0-9]+)*$/);
    }
  });
});
