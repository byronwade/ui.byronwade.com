...
> that already has its own theme is _not_ a clean `init`/`add`: `init` against foundation would overwrite
...
```bash
npx shadcn@latest add @byronwade/activity-ring
...
`add @byronwade/activity-ring` pulls `status-dot` + `utils`; `add @byronwade/gradient-avatar` pulls `identity`; etc.
...
> shadcn's generic `add --all` flag adds all items from the _default_ shadcn/ui registry, **not** a custom
...
```css
:root {
  --brand: oklch(0.55 0.2 25);
} /* e.g. a red-orange */
.dark {
  --brand: oklch(0.65 0.2 25);
}
```
