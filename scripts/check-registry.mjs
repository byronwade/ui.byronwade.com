...
if (dupes.length)
  fail(`Duplicate item names: ${[...new Set(dupes)].join(", ")}`)
...
if (!foundation) fail("Missing required `foundation` item")
  ...
  if (foundation.type !== "registry:base")
    fail("foundation must have type registry:base")
  for (const key of ["light", "dark", "theme"]) {
    if (!foundation.cssVars?.[key])
      fail(`foundation.cssVars missing \`${key}\``)
  }
}
...
for (const item of items) {
  ...
  for (const dep of item.registryDependencies ?? []) {
    ...
    if (!nameSet.has(resolved)) {
      fail(
        `${item.name}: registryDependency \`${dep}\` does not match any item name`,
      )
    }
  }
  ...
  for (const f of item.files ?? []) {
    ...
    if (item.type === "registry:component" && subdir !== "components") {
      fail(
        `${item.name}: component files must live under registry/components/: ${f.path}`,
      )
    }
    ...
    if (
      ...
      !f.target.startsWith("components/")
    ) {
...
if (checkBuilt) {
  ...
  if (!existsSync(outDir)) {
    ...
    const expected = items
      .filter((i) => i.name !== "registry")
      .map((i) => `${i.name}.json`)
    const builtSet = new Set(built)
    const missingBuilt = expected.filter((f) => !builtSet.has(f))
    const extraBuilt = built.filter(
      (f) => !expected.includes(f) && f !== "registry.json",
    )

    if (missingBuilt.length) {
      fail(
        `public/r/ missing built items:\n${missingBuilt.map((f) => `  - ${f}`).join("\n")}`,
      )
    }
    if (extraBuilt.length) {
      warn(
        `public/r/ has unexpected files:\n${extraBuilt.map((f) => `  - ${f}`).join("\n")}`,
      )
    }
  }
}
...
// ── Report ───────────────────────────────────────────────────────────────────
console.log(
  `check-registry: ${items.length} items, ${referencedPaths.size} source file(s)`,
)
