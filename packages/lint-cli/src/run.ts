...
export interface RunOptions { fix?: boolean; maxColorDistance?: number; offSystemComponents?: "warn" | "error" | "off"; errorDetectors?: string[]; cwd?: string; }
export interface RunResult { errorCount: number; warnCount: number; files: { file: string; violations: Violation[] }[]; }

export const isBlocking = (v: Violation, errorDetectors?: string[]): boolean =>
  v.severity === "error" && (!errorDetectors || errorDetectors.includes(v.detector));
...
export async function run(patterns: string[], opts: RunOptions = {}): Promise<RunResult> {
  ...
  for (const file of files) {
    ...
    for (const v of violations) isBlocking(v, opts.errorDetectors) ? result.errorCount++ : result.warnCount++;
