...
function Skin({ hue }: { hue: number }) {
  ...
  return (
    <div style={scope} className="space-y-3 rounded-2xl edge bg-card p-4">
      ...
      <p className="font-mono text-xl tabular-nums leading-none text-foreground">$48.2k</p>
      <div className="flex h-9 items-end gap-1">
        {BARS.map((h, i) => (
          <span key={i} className="flex-1 rounded-sm bg-chart-1" style={{ height: `${h}%` }} />
        ))}
      </div>
    ...
    <article className="max-w-none">
      ...
      <section className={`relative ${BLEED} overflow-hidden border-b border-border`}>
        ...
        <div className="relative py-12 sm:py-16">
          ...
          <h1 className="mt-5 max-w-3xl text-[clamp(2.5rem,7vw,5rem)] font-normal leading-[0.95] tracking-tight text-foreground text-balance">
            One variable re-skins <span className="text-brand">everything.</span>
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-muted-foreground text-pretty">
            The accent isn&apos;t scattered through components — it&apos;s derived from a single
            token, <span className="text-foreground">--brand</span>. Override it once and rings,
            charts, success, buttons, and active states all follow, light and dark. Below is the{" "}
            <span className="text-foreground">same card</span>, eight accents apart — each just one
            line of CSS.
          </p>
          ...
          <div className="mt-10 flex items-baseline justify-between gap-4">
            ...
            <p className="hidden font-mono text-[11px] text-muted-foreground sm:block">↓ one knob</p>
          </div>
      ...
      <section className="py-16">
        <div className="grid gap-10 lg:grid-cols-[1fr_1.2fr] lg:items-center">
          <div>
            <p className="font-mono text-xs uppercase tracking-[0.2em] text-brand">The whole change</p>
            ...
            <p className="mt-4 text-[15px] leading-relaxed text-muted-foreground text-pretty">
              No component edits, no theme fork. Pick a slightly lighter, more chromatic value for
              dark so the accent keeps its punch against the warm dark surface.
            </p>
          </div>
          <CodeBlock
            lang="css"
...
.dark { --brand: oklch(0.65 0.20 25); }   /* dark  */`}
      ...
      <section className={`${BLEED} border-y border-border bg-card`}>
        <div className="py-16">
          ...
          <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-muted-foreground text-pretty">
            <code className="font-mono text-[13px] text-foreground">oklch(L C H)</code> — perceptual{" "}
            <span className="text-foreground">lightness</span>,{" "}
            <span className="text-foreground">chroma</span>, and{" "}
            <span className="text-foreground">hue</span>. Each axis moves independently, so tuning an
          ...
          <div className="mt-8 space-y-6">
            {[
              ...
              },
              {
                ...
                stops: [0, 0.04, 0.08, 0.12, 0.16, 0.2].map((c) => `oklch(0.62 ${c} 148)`),
              },
              {
                k: "H · hue",
                hint: "0 → 360 · which color",
                stops: [20, 90, 150, 210, 270, 330].map((h) => `oklch(0.62 0.18 ${h})`),
              },
            ].map((axis) => (
              <div key={axis.k} className="grid gap-2 sm:grid-cols-[10rem_1fr] sm:items-center">
                <div>
                  <p className="font-mono text-sm text-foreground">{axis.k}</p>
                  <p className="font-mono text-[11px] text-muted-foreground">{axis.hint}</p>
                </div>
      ...
      {/* ============================ PRESETS ========================= */}
      <section className="py-16">
        <p className="font-mono text-xs uppercase tracking-[0.2em] text-brand">Ready-to-paste accents</p>
        <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-muted-foreground text-pretty">
          Drop any of these into <code className="font-mono text-[13px] text-foreground">--brand</code>
          . Each is tuned to sit right on the warm canvas.
        </p>
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {PRESETS.map((p) => (
            <div key={p.name} className="flex items-center gap-4 rounded-2xl edge bg-card p-4">
              <span
              ...
              <div className="min-w-0">
                ...
                <p className="truncate font-mono text-[11px] text-muted-foreground">{p.value}</p>
              </div>
            </div>
          ))}
        </div>
        <div className="mt-6">
          <CodeBlock lang="css" code={`/* e.g. Ocean */\n:root { --brand: oklch(0.60 0.15 235); }\n.dark { --brand: oklch(0.70 0.15 235); }`} />
        </div>
      </section>
      ...
      <section className={`${BLEED} border-y border-border bg-card`}>
        <div className="py-16">
          <p className="font-mono text-xs uppercase tracking-[0.2em] text-brand">The cascade</p>
          <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-muted-foreground text-pretty">
            These tokens all resolve to <span className="text-foreground">--brand</span>. Change the
            one and the family moves together.
          </p>
          <div className="mt-8 grid gap-px overflow-hidden rounded-2xl edge bg-border sm:grid-cols-2">
            {[
              { token: "--brand", swatch: "bg-brand", label: "Buttons, links, brand surfaces" },
              { token: "--ring", swatch: "bg-ring", label: "Focus rings on every element" },
              { token: "--success", swatch: "bg-success", label: "Positive status + gauges" },
              { token: "--chart-1", swatch: "bg-chart-1", label: "Primary chart series" },
            ].map((f) => (
              <div key={f.token} className="flex items-center gap-4 bg-card p-5">
                <span className={`size-10 shrink-0 rounded-xl edge ${f.swatch}`} />
                <div className="min-w-0">
                  <p className="font-mono text-sm text-foreground">{f.token}</p>
                  <p className="text-sm text-muted-foreground text-pretty">{f.label}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="grid gap-10 py-16 lg:grid-cols-2">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.2em] text-brand">Fixed exceptions</p>
          <p className="mt-3 text-[15px] leading-relaxed text-muted-foreground text-pretty">
            Two palettes carry meaning, so they <span className="text-foreground">don&apos;t</span>{" "}
            follow --brand.
          </p>
          <div className="mt-6 space-y-4">
            <div className="rounded-2xl edge bg-card p-5">
              <p className="font-mono text-xs text-muted-foreground">--chart-2 … --chart-5</p>
              <div className="mt-3 flex items-end gap-2">
                {["bg-chart-1", "bg-chart-2", "bg-chart-3", "bg-chart-4", "bg-chart-5"].map((c, i) => (
                  <span key={c} className={`flex-1 rounded-md edge ${c}`} style={{ height: `${52 - i * 6}px` }} />
                ))}
              </div>
            </div>
            <div className="rounded-2xl edge bg-card p-5">
              <p className="font-mono text-xs text-muted-foreground">--activity-* (agent pastels)</p>
              <div className="mt-3 grid grid-cols-4 gap-2">
                {["bg-activity-thinking", "bg-activity-search", "bg-activity-read", "bg-activity-edit"].map(
                  (a) => (
        ...
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.2em] text-brand">Do &amp; don&apos;t</p>
          <div className="mt-6 space-y-3">
            {[
              { ok: true, t: "Override --brand (and its .dark value) in globals.css." },
              { ok: true, t: "Tone with opacity — bg-brand/10 — for soft chips and rings." },
              { ok: false, t: "Hardcode a hex or Tailwind color in a component." },
              { ok: false, t: "Pin --ring / --chart-1 / --success to a literal color." },
            ].map((d) => (
              <div
                key={d.t}
              ...
              >
                ...
                <span className={d.ok ? "text-foreground" : "text-muted-foreground"}>{d.t}</span>
              </div>
            ))}
          </div>
          <p className="mt-5 text-sm leading-relaxed text-muted-foreground text-pretty">
            Authored in <span className="text-foreground">oklch</span>, so tints, dark values, and
            contrast stay honest across every accent.
          </p>
        </div>
      </section>
