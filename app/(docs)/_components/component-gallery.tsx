...
function SurfaceBadge({ surface }: { surface: CatalogItem["surface"] }) {
  return (
    <span
      className={cn(
...
export function ComponentGallery({
  ...
  const scopedItems = React.useMemo(
    () =>
      surface === "all" ? items : items.filter((i) => i.surface === surface),
    [items, surface],
  )

  const allGroups = React.useMemo(
    () => uniqueSorted(scopedItems.map((i) => i.group)),
    [scopedItems],
  )
  const allTags = React.useMemo(
    () => uniqueSorted(scopedItems.flatMap((i) => i.tags)),
    [scopedItems],
  )
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {surface !== "all" && (
            <ActivePill
              label={surfaceShortLabel(surface)}
