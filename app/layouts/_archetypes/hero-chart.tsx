...
const kpis = [
  { label: "MRR", value: "$101k", icon: CreditCard, dir: "up" as const, delta: "14.7%", data: [42, 51, 47, 63, 58, 72, 69, 78, 85, 92, 88, 101], tone: "text-brand" },
  { label: "Active users", value: "72.4k", icon: Users, dir: "up" as const, delta: "12.5%", data: [28, 31, 30, 38, 41, 44, 47, 52, 55, 61, 64, 72], tone: "text-brand" },
  { label: "Conversion", value: "4.8%", icon: Activity, dir: "up" as const, delta: "0.3%", data: [3.9, 4.0, 4.1, 4.0, 4.3, 4.4, 4.5, 4.4, 4.6, 4.7, 4.7, 4.8], tone: "text-success" },
  { label: "Churn", value: "1.9%", icon: Zap, dir: "down" as const, delta: "0.2%", data: [2.6, 2.5, 2.5, 2.3, 2.4, 2.2, 2.2, 2.1, 2.1, 2.0, 2.0, 1.9], tone: "text-success" },
];
...
const products = [
  { name: "Pro subscription", sku: "SUB-PRO", revenue: "$48,200", trend: "up" as const, share: "47%" },
  { name: "Team seats", sku: "SUB-TEAM", revenue: "$31,940", trend: "up" as const, share: "31%" },
  { name: "Usage overage", sku: "USG-OVR", revenue: "$14,110", trend: "down" as const, share: "14%" },
  { name: "Add-ons", sku: "ADD-ONS", revenue: "$8,300", trend: "up" as const, share: "8%" },
];
...
export function HeroChartArchetype() {
  ...
  return (
    <div className="relative min-h-dvh overflow-hidden bg-background text-foreground">
      <div aria-hidden className="bg-grid pointer-events-none absolute inset-0 opacity-50 [mask-image:radial-gradient(70%_50%_at_50%_0%,#000,transparent)]" />
          ...
          <div className="relative flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <div className="mt-1 flex items-end gap-3">
                <span className="text-gradient-brand text-5xl font-semibold tracking-tight tabular-nums">$101k</span>
              ...
              <p className="mt-1 text-xs text-muted-foreground">vs. $88k last month · trailing {range === "6m" ? "6" : "12"} months</p>
            </div>
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              ...
              <span className="flex items-center gap-1.5">
                <span className="size-2 rounded-full bg-[var(--chart-3)]" /> Active users
              </span>
            </div>
          </div>

          <ChartContainer config={chartConfig} className="relative mt-6 h-72 w-full">
            <AreaChart data={shown} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              <defs>
                ...
                <linearGradient id="fillActive" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="var(--color-active)" stopOpacity={0.18} />
              ...
              <Area type="monotone" dataKey="revenue" stroke="var(--color-revenue)" fill="url(#fillRevenue)" strokeWidth={2.5} />
            </AreaChart>
          </ChartContainer>
        </section>
              ...
              {sources.map((s) => (
                <div key={s.name} className="space-y-1.5">
                  <div className="flex items-center justify-between text-sm">
                    <span>{s.name}</span>
                    <span className="font-mono tabular-nums text-muted-foreground">{s.revenue}</span>
                  </div>
                  <div className="h-1.5 overflow-hidden rounded-full bg-muted">
                    <div className="h-full rounded-full bg-brand" style={{ width: `${s.share}%` }} />
                  </div>
                </div>
              ))}
            </div>
                ...
                {products.map((p) => (
                  <TableRow key={p.sku}>
                    ...
                    <TableCell className="pr-5 text-right">
                      <Badge variant={p.trend === "up" ? "success" : "destructive"}>{p.share}</Badge>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
