...
const nav: { group: string; items: { id: Pane; label: string; icon: typeof Settings2 }[] }[] = [
  {
    group: "Workspace",
    items: [
...
const members = [
  { name: "Ana Reyes", email: "ana@acme.com", role: "Owner", tone: "success" as const, status: "Active" },
  { name: "Liam Cho", email: "liam@acme.com", role: "Admin", tone: "success" as const, status: "Active" },
  { name: "Priya Nair", email: "priya@acme.com", role: "Member", tone: "warning" as const, status: "Invited" },
];
...
const paneTitle: Record<Pane, { title: string; description: string }> = {
  general: { title: "General", description: "Basic information about your workspace." },
  members: { title: "Members", description: "Manage who has access to this workspace." },
  billing: { title: "Billing", description: "Plan, usage, and invoices." },
  notifications: { title: "Notifications", description: "Choose what lands in your inbox." },
  security: { title: "Security", description: "Authentication and active sessions." },
};
    ...
    <div className="flex h-dvh bg-background text-foreground">
      ...
      <aside className="hidden w-64 shrink-0 flex-col border-r border-border bg-card/40 md:flex">
        ...
        <div className="flex items-center gap-3 px-4 py-4">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-muted-foreground">Team plan · 12 seats</p>
          </div>
        </div>

        <nav aria-label="Settings sections" className="flex-1 space-y-4 overflow-y-auto px-2 pb-4">
          {nav.map((g) => (
            <div key={g.group}>
      ...
      <div className="relative flex min-w-0 flex-1 flex-col">
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className="mx-auto max-w-2xl px-6 py-10">
            <header className="mb-8">
              ...
              <p className="mt-1 text-sm text-muted-foreground">{head.description}</p>
            </header>

            <div className="space-y-8">
              {pane === "general" && (
                <>
                  <Section title="Workspace" description="How your workspace appears across Acme.">
                    <div className="space-y-4 p-5">
                      <div className="space-y-1.5">
                        <Label htmlFor="ws-name">Name</Label>
                        <Input id="ws-name" defaultValue="Acme Inc." onChange={touch} />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Select defaultValue="us" onValueChange={touch}>
                          ...
                          <SelectContent>
                            <SelectItem value="us">US East (N. Virginia)</SelectItem>
                  ...
                  <Section title="Preferences" description="Defaults applied across the workspace.">
                    <SettingsList>
                      <SettingRow title="Compact mode" description="Reduce spacing for dense screens." control={<Switch onCheckedChange={touch} />} />
                      <SettingRow title="Usage analytics" description="Share anonymous usage data to help improve the product." control={<Switch defaultChecked onCheckedChange={touch} />} />
                      <SettingRow title="Beta features" description="Get early access to features still in development." control={<Switch onCheckedChange={touch} />} />
                    </SettingsList>
                  </Section>
                </>
              )}
              ...
              {pane === "members" && (
                <Section title="Members" action={<Button size="sm">Invite</Button>}>
                  <div className="divide-y divide-border">
                    {members.map((m) => (
                      <div key={m.email} className="flex items-center justify-between gap-3 px-5 py-4">
                        ...
                        <div className="flex items-center gap-3">
                          <span className="hidden text-xs text-muted-foreground sm:inline">{m.role}</span>
              ...
              {pane === "billing" && (
                <SplitWithRail
                  summary={
                    <Section title="Plan" description="You are on the Team plan, billed monthly.">
                      <div className="space-y-4 p-5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-sm font-medium">Team · $40 / seat</div>
                            <div className="text-xs text-muted-foreground">Renews Jul 1, 2026</div>
                          </div>
                        </div>
                        <div className="space-y-1.5">
              ...
              {pane === "notifications" && (
                <Section title="Email notifications" description="Choose what lands in your inbox.">
                  <SettingsList>
                    <SettingRow title="Incidents" description="Critical and high-severity alerts." control={<Switch defaultChecked onCheckedChange={touch} />} />
                    <SettingRow title="Deploys" description="When a release ships to production." control={<Switch defaultChecked onCheckedChange={touch} />} />
                    <SettingRow title="Weekly digest" description="A Monday summary of activity." control={<Switch onCheckedChange={touch} />} />
                  </SettingsList>
                </Section>
              )}
              ...
              {pane === "security" && (
                <>
                  <Section title="Authentication" description="Protect your account with an extra layer.">
                    <SettingsList>
                      <SettingRow title="Two-factor authentication" description="Require a one-time code at sign-in." control={<Switch defaultChecked onCheckedChange={touch} />} />
                      <SettingRow title="Password" description="Last changed 3 months ago." control={<Button variant="outline" size="sm">Change</Button>} />
                    </SettingsList>
                  </Section>
        ...
        >
          ...
          <div className={cn("flex items-center gap-2", dirty && "pointer-events-auto")}>
