...
const kpis = [
  { label: "Uptime", value: "99.98%", hint: "30-day", icon: Activity, data: series.uptime, dir: "up" as const, delta: "0.02%", tone: "text-success" },
  { label: "p95 latency", value: "142ms", hint: "last hour", icon: Timer, data: series.latency, dir: "down" as const, delta: "8ms", tone: "text-brand" },
  { label: "Error rate", value: "0.04%", hint: "last hour", icon: Zap, data: series.errors, dir: "down" as const, delta: "0.01%", tone: "text-success" },
  { label: "Throughput", value: "3.2k/s", hint: "requests", icon: Cpu, data: series.throughput, dir: "up" as const, delta: "320", tone: "text-brand" },
];

const regions: { code: string; tone: StatusTone; p95: string; load: number[] }[] = [
  { code: "us-east-1", tone: "success", p95: "142ms", load: [3, 4, 3, 5, 4, 6, 5, 6] },
  { code: "eu-west-1", tone: "success", p95: "119ms", load: [4, 3, 4, 4, 5, 4, 5, 5] },
  { code: "ap-south-1", tone: "warning", p95: "268ms", load: [5, 6, 7, 6, 8, 7, 9, 8] },
  { code: "us-west-2", tone: "success", p95: "98ms", load: [2, 3, 2, 3, 3, 4, 3, 4] },
  { code: "edge", tone: "success", p95: "41ms", load: [6, 6, 7, 6, 7, 7, 8, 7] },
];
...
export function GaugeArchetype() {
  return (
    <div className="relative min-h-dvh overflow-hidden bg-background text-foreground">
      {/* atmosphere */}
      <div aria-hidden className="bg-grid pointer-events-none absolute inset-0 opacity-60 [mask-image:radial-gradient(80%_55%_at_50%_0%,#000,transparent)]" />
      <div aria-hidden className="glow-brand pointer-events-none absolute inset-x-0 top-0 h-[420px]" />
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-14">
          ...
          {regions.map((r) => (
            <div key={r.code} className="flex flex-col gap-2 bg-card p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-medium tabular-nums">{r.p95}</span>
              </div>
              <Sparkline
                data={r.load}
                fill
                className={r.tone === "warning" ? "h-6 text-warning" : "h-6 text-brand"}
              />
            </div>
          ))}
        </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.5fr_1fr]">
          ...
          <section className="rounded-2xl bg-card p-5 shadow-card">
            ...
            <EventTimeline
              events={[
                { title: "Latency returned to baseline", tone: "success", timestamp: "12:04 UTC", description: "us-east-1 recovered after cache warm-up." },
                { title: "Elevated p95 in ap-south-1", tone: "warning", timestamp: "11:48 UTC", description: "Auto-scaled from 8 to 12 nodes." },
                { title: "Deploy v2.14.0", tone: "info", timestamp: "09:30 UTC", description: "Rolled out to all regions." },
                { title: "Nightly backup complete", tone: "neutral", timestamp: "04:00 UTC" },
              ]}
            />
          </section>
        </div>
