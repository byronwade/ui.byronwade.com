...
const views: { id: View; label: string; match: (s: Service) => boolean }[] = [
  { id: "all", label: "All services", match: () => true },
  {
    id: "production",
    label: "Production",
    match: (s) => s.env === "production",
  },
  {
    id: "degraded",
    label: "Needs attention",
    match: (s) => s.status === "warning" || s.status === "danger",
  },
...
export function RichInventoryArchetype() {
  ...
  const summary: { label: string; value: React.ReactNode; tone?: string }[] = [
    ...
    {
      label: "Healthy",
      value: services.filter((s) => s.status === "success").length,
      tone: "text-success",
    },
    {
      label: "Attention",
    ...
    {
      label: "Avg load",
      value: `${Math.round(services.reduce((a, s) => a + s.usage, 0) / services.length)}%`,
    },
  ]
    ...
    <div className="flex h-dvh bg-background text-foreground">
      ...
      <aside className="hidden w-60 shrink-0 flex-col border-r border-border bg-card/40 lg:flex">
        ...
        <nav aria-label="Saved views" className="flex flex-col gap-0.5 p-2">
          <p className="px-2 pb-1 pt-2 font-mono text-[11px] uppercase tracking-[0.14em] text-muted-foreground">
            Views
          </p>
          {views.map((v) => {
          ...
          <p className="px-2 pb-1 pt-4 font-mono text-[11px] uppercase tracking-[0.14em] text-muted-foreground">
            Regions
          </p>
          {["us-east-1", "eu-west-1", "ap-south-1", "global"].map((r) => (
            <div
              key={r}
              className="flex items-center gap-2 rounded-lg px-2.5 py-1.5 font-mono text-xs text-muted-foreground"
            >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 shrink-0 items-center justify-between gap-4 border-b border-border px-5">
          <div>
            ...
            <p className="hidden text-xs text-muted-foreground sm:block">
              Every service across your regions, at a glance.
            </p>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-5 py-3">
          ...
          <div className="flex items-center gap-2">
            <FilterPill icon={<SlidersHorizontal className="size-3.5" />}>
              Region
            </FilterPill>
            <SegmentedControl
              value={sort}
