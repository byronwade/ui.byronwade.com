...
// Provider link row — shared layout for every "Open in <provider>" entry. The
// link inherits token color (`currentColor` SVGs), and the trailing
// external-link glyph is muted so the provider title leads.
const openInLinkVariants = cva(
...
const OpenInProviderItem = ({
  ...
  className,
  ...props
}: OpenInItemProps & { provider: ProviderKey }) => {
    ...
    <DropdownMenuItem
      ...
      render={
        <a
          ...
          rel="noopener"
          target="_blank"
          className={openInLinkVariants()}
        />
      }
      {...props}
    >
      <span className="shrink-0" data-slot="open-in-item-icon">
        {icon}
      </span>
...
export type OpenInCursorProps = OpenInItemProps;
