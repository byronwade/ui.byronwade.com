 ...
 * Reworked to the byronwade/ui design system: token-friendly defaults
 * (transparent backing, no default tint), `cn()` + `className` passthrough, and
 * a `data-slot` hook. `backgroundColor`/`tintColor` are image-processing inputs
 * (any CSS color) applied to sampled pixels, not UI chrome; the per-dot
 * `rgb(...)` fills come from the source image data.
 */
"use client"
