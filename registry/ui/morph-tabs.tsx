  ...
  const Row = (
    <div className="flex h-16 items-stretch justify-around gap-1 px-2">
      {items.map((item) => {
        ...
        return (
          <button
            ...
            className={cn(
              "flex flex-1 flex-col items-center justify-center gap-1 rounded-full text-[11px] outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring/50",
              item.active
                ? "text-foreground"
      ...
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className="grid w-10 place-items-center rounded-full text-muted-foreground outline-none transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/50"
      >
